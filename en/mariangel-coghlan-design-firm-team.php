<?php 
$title="Conócenos | Mariangel Coghlan";
$desc = "Diseñadora optimista e incansable empresaria apasionada del color y la belleza. ";
$key = "diseño de interiores";
$page = 'nosotros';
$canonical = "https://www.mcoghlan.mx/conocenos";
include('../global/header-en.php');
?>	

      <!-- #header-wrap -->
 <section id="hero-interior" class="hero-conocenos">
 <div class="container">
		<div class="row">
			<div class="text-cont">
				<h2>CONÓCENOS</h2>
			</div>
		</div>
  </div>
 </section>  
<section id="nosotros-top" style="">
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				<div class="cont-text">
					<h1>Mariangel Coghlan Design Team</h1>
				</div>
			</div>
		</div>
	</div>
</section>
<section id="cajas-texto">
	<div class="container">
		<div class="row">
			<div class="col-md-4 pad-cajas">		
				<div class="img-cont servicios">
					<h4>WHO ARE WE?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="/img/conocenos/quienes-somos-384.jpg" >
					<p> Our firm is composed of a talented team of experts whose passion is transforming spaces. We love and fully enjoy what we do and are proud of the connection between us. We understand the importance of being up to date while continuously learning daily, we consider ourselves lucky creatives.</p>
				</div>
			</div>
			<div class="col-md-4 pad-cajas">
				<div class="img-cont servicios">
					<h4>WHAT DO WE DO?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="/img/conocenos/que-hacemos-350-x-350px.jpg">
					<p>We transform ordinary spaces into extraordinary places. Always designing with people in mind, we specialize in the design and execution of residential and commercial interior design projects, remodeling, and architectural design. Our goal is to surprise each of our clients and exceed their expectations.</p>
				</div>
			</div>
			<div class="col-md-4 pad-cajas">
				<div class="img-cont servicios">
					<h4>WHY DO WE DO IT?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="/img/conocenos/porque-lo-hacemos.jpg">
					<p>For the possibility, honor, and privilege of collaborating with each of our clients, helping them improve their lifestyle. We are convinced that living in a harmonious space improves our environment and contributes to our well-being. Working on something we are truly passionate about makes us happier.</p>
				</div>
			</div>
		</div>
	</div>
</section>
<section id="reconocimiento">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h3>Acknowledgement</h3>
				<p>As a team, we feel very honored to have received this distinction. For us, it is an honor to be recognized for doing something we are passionate about.</p>
				<p>Our priority has always been to provide exceptional service. We are very grateful for the recognition given to us by LUXURY LIFESTYLE AWARDS.</p>
				<img src="/img/conocenos/congratulatory-letter.jpg" alt="Interior Design">
			</div>
		</div>
	</div>
</section>
<?php include('global/ventajas.php');?>
<section id="bio-equipo">
	<div class="container">
		<div class="row pad-bio-mc">
			<div class="col-md-4">
				<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/mariangel1-nosotros.jpg" onmouseover="this.src='/img/conocenos/mariangel2-nosotros.jpg'" onmouseout="this.src='/img/conocenos/mariangel1-nosotros.jpg'" >
			</div>
			<div class="col-md-8">
				<h3>Mariangel Coghlan</h3>
				<p>Architect, founder of the design firm MARIANGEL COGHLAN. With over 25 years of experience, she has become a reference in Mexican interior design. Mariangel, mother of four and the spouse of a great artist, is a tireless and optimistic entrepreneur, passionate about color and beauty. She is constantly involved in endeavors that allow her to spread the value of design as an instrument of positive change in our society.</p>
				<p><a href="https://www.mcoghlan.mx/hubfs/CV-MARIANGELCOGHLAN.pdf" target="_blank">DESCARGA SU CV</a></p>
				<div class="datos-mc">
					<a href="mailto:mariangel@mcoghlan.mx"><img class="" src="/img/mail-3.jpg" alt="Mail"> </a>
					<a href="https://www.instagram.com/mariangelcoghlan/?hl=es-la"><img src="/img/ig.jpg" alt="Mail"> </a>
				</div>
			</div>
			
		</div>
		<div class="row">
			<div class="col-12 title-eqp">
				<h3>Meet our team</h3>
			</div>
		</div>
		<div class="equipo-slider neg-mar">	
			<div class="pad-slides">
				<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/daniela2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/daniela1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Daniela Dorantes<span>Project manager</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:daniela@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/omar2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/omar1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Omar Muñoz <span>Finance Director</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:omar@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/mario2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/mario1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Mario Santos<span>Project manager</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:mario@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/edwin2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/edwin1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Jesica Garcés<span>Project manager</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:jesica@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
	

			<div class="pad-slides">
				<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/cynthia2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/cynthia1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Cynthia Mendoza<span>Project manager</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:cynthia@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/enrique2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/enrique1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Enrique Rivera <span>Operations manager</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:enrique@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/natalia2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/natalia1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Natalia Tornel<span>Project leader</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:natalia@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="/img/conocenos/edwin2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="/img/conocenos/edwin1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Edwin Elos<span>Administration</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:edwin@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
		</div>	
	</div>
</section>
<section id="val-resp">
	<div class="container">
		<div class="row">
			<div class="col-12 top">
				<h2>Valores</h2>
				<p>We are aware that accomplishing extraordinary things can be difficult; yet we strive to do, on a daily basis, ordinary things extraordinarily well. This is how we work at our firm, offering the best service is always a priority. To achieve this it is necessary to live by certain values. Our team has defined five indispensable values in our professional activities: integrity, excellence, passion, creativity and happiness.</p>
				<p>In light of these values, we have made many difficult decisions. Remaining true to them has allowed us to define our position and maintain a consistent style in which our priority is to put other individuals, their well-being and their benefit first. Our priority is always to help each of our clients to live better.</p>
			</div>
		</div>
		<div class="row valores">
			<div class="col-md-4">
				<h3>INTEGRIDAD</h3>
				<img src="/img/conocenos/integridad.jpg" alt="Interior Design">
				<p>Integrity expresses itself as righteousness, honesty, respect for others, correctness, responsibility, emotional control, self-respect, punctuality, loyalty, neatness, discipline, congruence and steadfastness in one's actions. Integrity is doing the right thing for the right reasons and in the right way.</p>
			</div>		
			<div class="col-md-4">
				<h3>EXCELLENCE</h3>
				<img src="/img/conocenos/excelencia.jpg" alt="Interior Design">
				<p>Our team strives for excellence in our professional work, and we know that achieving it implies commitment, dedication and daily effort. We fully identify with Aristotle's definition "We are what we do day by day. So excellence is not an act, it is a habit." That is why we always strive to achieve it.</p>
			</div>		
			<div class="col-md-4">
				<h3>PASIÓN</h3>
				<img src="/img/conocenos/pasion.jpg" alt="Interior Design">
				<p>Passion is the reality that defines us, that is, it expresses a significant aspect of our identity as well as our personality. We consider ourselves passionate and enthusiastic about what we do professionally, we are convinced that this passion enhances the human condition and is the seasoning that leads us to materialize the greatest achievements.</p>
			</div>
			<div class="col-md-4">
				<h3>CREATIVIDAD</h3>
				<img src="/img/conocenos/creatividad.jpg" alt="Interior Design">
				<p>Creativity is the ability to generate new ideas or concepts, new associations between known ideas and concepts, which usually produce original solutions. Creativity is synonymous with original thinking, constructive imagination, divergent thinking or creative thinking, undoubtedly indispensable for design.</p>
			</div>
			<div class="col-md-4">
				<h3>FELICIDAD</h3>
				<img src="/img/conocenos/felicidad-1.jpg" alt="Interior Design">
				<p id="resp-social">Happiness is a way of life, it consists of learning to see your environment with a filter that brings us feelings of well-being and balance. Having reasons to get up every morning with the clear idea of helping people to live better with the illusion of contributing to create a more beautiful and humane world is something that fills us with happiness.</p>
			</div>		
		</div>
		<div class="row pad-top">
			<div class="col-12 top">
				<h2>Social Responsibility</h2>				
			</div>
		</div>
		<div class="row valores">
			<div class="col-md-4">
				<h3>INTEGRITY</h3>
				<img src="/img/conocenos/certificacion.jpg" alt="Interior Design">
				<p>Each piece designed in the firm, MARIANGEL COGHLAN, is manufactured with the highest quality standards, they are made following a protocol of social and environmental responsibility, the raw materials we use have been obtained from organic crops or have been manufactured in an environmentally friendly way.</p>
			</div>		
			<div class="col-md-4">
				<h3>EXCELLENCE</h3>
				<img src="/img/conocenos/medioaambiente.jpg" alt="Interior Design">
				<p>In the firm MARIANGEL COGHLAN we work to develop our activities in a respectful way with the conservation of the environment, from the perspective that the care of the natural environment and sustainable development provide added value to our activities and services, consequently to the quality of life of our customers and society in general. We are committed to the environment so we modernize our facilities to reduce the consumption of water, electricity and all kinds of office materials, raising awareness in our team to reduce the environmental impact.</p>
			</div>		
			<div class="col-md-4">
				<h3>SOCIAL RESPONSIBILITY</h3>
				<img src="/img/conocenos/responsabilidadsocial.jpg" alt="Interior Design">
				<p>At MARIANGEL COGHLAN we have a corporate policy on social responsibility, for which we consider one of the most valuable assets of the company, our reputation. Being a leading company, conducting ourselves with honor, and having the highest level of integrity and corporate ethics are an important part of our firm's objectives. Maintaining this reputation is one of our most significant tasks, which requires us to carry out our work by living our values with intensity, adhering to ethical principles, as well as being attentive to the needs of those around us.</p>
			</div>		
		</div>
	</div>
</section>
<?php // include('../global/cta-ebooks.php');?>
<!-- <section id="unete">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h2>Únete al equipo</h2>
			</div>
			<div class="col-md-6">
				<h4>Nuestra Firma trabaja diariamente para ser la mejor, por lo mismo solamente contratamos a los mejores. Tú puedes formar parte de este increíble equipo.</h4>
				<p>Vacantes</p>
				<ul>
					<li><a href="https://www.mcoghlan.mx/hubfs/lider-de-proyecto-vacante-mariangelcoghlan.pdf" target="_blank">Líder de Proyecto</a></li>
					<li><a href="https://www.mcoghlan.mx/hubfs/disenador-jr.pdf" target="_blank">Diseñador Jr.</a></li>
					<li><a href="https://www.mcoghlan.mx/hubfs/practicas-vacante-mariangelcoghlan.pdf" target="_blank">Prácticas profesionales</a></li>
					<li><a href="https://www.mcoghlan.mx/hubfs/AUXILIAR%20ADMINISTRATIVO.pdf" target="_blank">Auxiliar Administrativo</a></li>
				</ul>
			</div>
			<div class="col-md-6">
				<p>Si estás interesad@ en postularte para ser parte de nuestra firma puedes mandar un mail a <a href="mailto:reclutamiento@mcoghlan.mx"> reclutamiento@mcoghlan.mx</a> con las siguientes características:</p>
				<ol>
					<li>Título del correo: (Según la vacante a la que aspiras)</li>
					<li>Adjuntar CV con foto actualizado</li>
					<li>Link para consultar portafolio</li>
					<li> Cinco líneas como máximo explicando por que habríamos de considerarte para formar parte de nuestro equipo.</li>
				</ol>
				<p>Antes de enviar un correo asegúrate de cumplir con todos los requisitos.</p>
			</div>
		</div>
	</div>
</section> -->
<?php include('../global/cta-visita-en.php');?>	
<?php 
include('../global/footer.php');
?>	

    