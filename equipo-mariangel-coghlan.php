<?php 
$title="El equipo de la firma | Mariangel Coghlan";
$desc = "Te mostramos a los apasionados integrantes del equipo de la Firma de Diseno Mariangel Coghlan";
$key = "diseño de interiores";
$page = 'nosotros';
$canonical = "https://www.mcoghlan.mx/equipo-mariangel-coghlan";
include('global/header.php');
?>	

      <!-- #header-wrap -->
 <section id="hero-interior" class="hero-conocenos">
 <div class="container">
		<div class="row">
			<div class="text-cont">
				<h2>CONÓCENOS</h2>
			</div>
		</div>
  </div>
 </section>  
<section id="nosotros-top" style="">
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				<div class="cont-text">
					<h1>Equipo de la firma de diseño Mariangel Coghlan</h1>
				</div>
			</div>
		</div>
	</div>
</section>
<section id="cajas-texto">
	<div class="container">
		<div class="row">
			<div class="col-md-4 pad-cajas">		
				<div class="img-cont servicios">
					<h4>¿QUIÉNES SOMOS?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="img/conocenos/quienes-somos-384.jpg" >
					<p><b> Somos un equipo de expertos apasionados por transformar espacios.</b> Amamos y disfrutamos plenamente lo que hacemos y estamos orgullosos del vínculo que tenemos entre nosotros. Estamos convencidos de la importancia de estar actualizados y seguir aprendiendo cada día por lo que nos consideramos creativos afortunados.</p>
				</div>
			</div>
			<div class="col-md-4 pad-cajas">
				<div class="img-cont servicios">
					<h4>¿QUÉ HACEMOS?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="img/conocenos/que-hacemos-350-x-350px.jpg">
					<p><strong>Transformamos espacios ordinarios en lugares extraordinarios.</strong> Diseñamos siempre pensando en las personas, nos especializamos en el diseño y la ejecución de proyectos de interiorismo residencial, comercial, remodelaciones y diseño arquitectónico. Nuestro interés es <strong>sorprender a cada uno de nuestros clientes y exceder sus expectativas.</strong></p>
				</div>
			</div>
			<div class="col-md-4 pad-cajas">
				<div class="img-cont servicios">
					<h4>¿POR QUÉ LO HACEMOS?</h4>
					<img class="" alt="Servicios Mariangel Coghlan" src="img/conocenos/porque-lo-hacemos.jpg">
					<p><strong>Por la posibilidad, el honor y el privilegio de colaborar con cada uno de nuestros clientes</strong> para mejorar su estilo de vida. Estamos convencidos de que habitar un espacio armonioso mejora nuestro entorno y contribuye a nuestra felicidad. Trabajar en algo que nos apasiona nos hace más felices.</p>
				</div>
			</div>
		</div>
	</div>
</section>
<section id="reconocimiento">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h3>Reconocimiento</h3>
				<p>Nos sentimos como equipo muy honrados al haber recibido esta distinción, para nosotros es un honor ser reconocidos por realizar algo que nos apasiona.</p>
				<p>Nuestra prioridad siempre ha sido dar un servicio excepcional. Estamos muy agradecidos por el reconocimiento que nos otorgó LUXUURY LIFESTYLE AWARDS.</p>
				<img src="img/conocenos/congratulatory-letter.jpg" alt="Diseño de Interiores">
			</div>
		</div>
	</div>
</section>
<?php include('global/ventajas.php');?>
<section id="bio-equipo">
	<div class="container">
		<div class="row pad-bio-mc">
			<div class="col-md-4">
				<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/mariangel1-nosotros.jpg" onmouseover="this.src='img/conocenos/mariangel2-nosotros.jpg'" onmouseout="this.src='img/conocenos/mariangel1-nosotros.jpg'" >
			</div>
			<div class="col-md-8">
				<h3>Mariangel Coghlan</h3>
				<p>Arquitecta, fundadora de la firma de diseño <b> MARIANGEL COGHLAN</b>. Con más de 25 años de experiencia se ha convertido en un referente del interiorismo Mexicano. Mariangel, madre de cuatro hijos y esposa de un gran artista, es una empresaria incansable y optimista apasionada del color y la belleza. Constantemente se involucra en iniciativas que le permitan difundir el valor del diseño como instrumento de cambio positivo en nuestra sociedad.</p>
				<p><a href="/pdf/CV-MARIANGELCOGHLAN.pdf" target="_blank">DESCARGA SU CV</a></p>
				<div class="datos-mc">
					<a href="mailto:mariangel@mcoghlan.mx"><img class="" src="img/mail-3.jpg" alt="Mail"> </a>
					<a href="https://www.instagram.com/mariangelcoghlan/?hl=es-la"><img src="img/ig.jpg" alt="Mail"> </a>
				</div>
			</div>
			
		</div>
		<div class="row">
			<div class="col-12 title-eqp">
				<h3>Nuestro equipo</h3>
			</div>
		</div>
		<div class="equipo-slider neg-mar">	
			<div class="pad-slides">
				<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/daniela2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/daniela1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Daniela Dorantes<span>Gerente de proyecto</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:daniela@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/omar2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/omar1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Omar Muñoz <span>Director de finanzas</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:omar@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/mario2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/mario1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Mario Santos<span>Gerente de proyecto</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:mario@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/edwin2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/edwin1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Jesica Garcés<span>Gerente de proyecto</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:jesica@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
	

			<div class="pad-slides">
				<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/cynthia2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/cynthia1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Cynthia Mendoza<span>Gerente de proyecto</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:cynthia@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/enrique2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/enrique1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Enrique Rivera <span>Gerente de operaciones</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:enrique@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/natalia2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/natalia1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Natalia Tornel<span>Líder de proyecto</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:natalia@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
			<div class="pad-slides">
			<div class="cont-img">
					<img class="img-eq hover" alt="Mariangel Coghlan" src="img/conocenos/edwin2.jpg" >
					<img class="img-mc" alt="Mariangel Coghlan" src="img/conocenos/edwin1.jpg" >
				</div>
				<div class="txt-cont">
					<p>Edwin Elos<span>Administración</span></p>
					<div>
						<a href="tel:5556619385"><i class="fa fa-phone"></i></a>
						<a href="mailto:edwin@mcoghlan.mx"><i class="fa fa-envelope"></i></a>
					</div>
				</div>
			</div>
		</div>	
	</div>
</section>
<section id="val-resp">
	<div class="container">
		<div class="row">
			<div class="col-12 top">
				<h2>Valores</h2>
				<p>Somos conscientes de que es complicado realizar cosas extraordinarias pero sabemos que podemos todos los días esforzarnos por hacer cosas ordinarias, extraordinariamente bien. Así trabajamos en nuestra firma, porque para nosotros ofrecer el mejor servicio siempre es prioridad. Estamos conscientes de que para lograrlo es necesario vivir cabalmente ciertos valores, hemos definido cinco valores indispensables en nuestro actuar profesional: integridad, excelencia, pasión, creatividad y felicidad.</p>
				<p>A la luz de estos valores hemos tomado muchas decisiones complicadas, permanecer fieles a ellos nos ha permitido definir nuestra postura y mantener un estilo coherente en el que nuestra prioridad es pensar primero en los otros, en su bienestar y en su beneficio. Nuestra prioridad siempre es ayudar a cada uno de nuestros clientes a vivir mejor.</p>
			</div>
		</div>
		<div class="row valores">
			<div class="col-md-4">
				<h3>INTEGRIDAD</h3>
				<img src="img/conocenos/integridad.jpg" alt="Diseño de Interiores">
				<p>La integridad se traduce como honradez, honestidad, respeto por los demás, corrección, responsabilidad, control emocional, respeto por sí mismo, puntualidad, lealtad, pulcritud, disciplina, congruencia y firmeza en las acciones. Integridad es hacer lo correcto por las razones correctas y del modo correcto.</p>
			</div>		
			<div class="col-md-4">
				<h3>EXCELENCIA</h3>
				<img src="img/conocenos/excelencia.jpg" alt="Diseño de Interiores">
				<p>Siempre buscamos la excelencia en nuestro quehacer profesional, sabemos que conseguirla implica compromiso, dedicación y un esfuerzo diario. Nos identificamos plenamente con la definición de Aristoteles “Somos lo que hacemos día a día. De modo que la excelencia no es un acto, es un hábito.” Por eso luchamos siempre por alcanzarla.</p>
			</div>		
			<div class="col-md-4">
				<h3>PASIÓN</h3>
				<img src="img/conocenos/pasion.jpg" alt="Diseño de Interiores">
				<p>La pasión es una realidad que nos autodefine, es decir, que expresa un rasgo significativo de nuestra identidad, de nuestra personalidad. Nos consideramos apasionados y entusiasmados por nuestro que hacer profesional, estamos convencidos de que esta pasión enaltecen la condición humana y constituyen el condimento que nos lleva a materializar los logros más grandes.</p>
			</div>
			<div class="col-md-4">
				<h3>CREATIVIDAD</h3>
				<img src="img/conocenos/creatividad.jpg" alt="Diseño de Interiores">
				<p>La creatividad es la capacidad de generar nuevas ideas o conceptos, de nuevas asociaciones entre ideas y conceptos conocidos, que habitualmente producen soluciones originales. La creatividad es sinónimo del pensamiento original, la imaginación constructiva, el pensamiento divergente o el pensamiento creativo, sin lugar a dudas indispensable para diseñar.</p>
			</div>
			<div class="col-md-4">
				<h3>FELICIDAD</h3>
				<img src="img/conocenos/felicidad-1.jpg" alt="Diseño de Interiores">
				<p id="resp-social">La felicidad es una forma de vivir en el mundo, consiste en aprender a ver la vida con un filtro que nos aporte sentimientos de bienestar y equilibrio. Tener motivos por los que levantarse cada mañana con la idea clara de ayudar a las personas a vivir mejor con la ilusión de contribuir a crear un mundo más bello y más humano es algo que nos llena de felicidad.</p>
			</div>		
		</div>
		<div class="row pad-top">
			<div class="col-12 top">
				<h2>Responsabilidad Social</h2>				
			</div>
		</div>
		<div class="row valores">
			<div class="col-md-4">
				<h3>INTEGRIDAD</h3>
				<img src="img/conocenos/certificacion.jpg" alt="Diseño de Interiores">
				<p>Cada pieza diseñada en la firma, MARIANGEL COGHLAN, está fabricada con los más altos estándares de calidad, se realizan siguiendo un protocolo de responsabilidad social y medioambiental, las materias primas que utilizamos se han obtenido de cultivos ecológicos o se han fabricado de forma respetuosa con el medio ambiente.<br>Toda la madera que empleamos para la elaboración de nuestros productos cuenta con la certificación FSC (Forest Stewardship Council), que garantiza una gestión forestal económicamente viable y socialmente beneficiosa y ambientalmente responsable.</p>
			</div>		
			<div class="col-md-4">
				<h3>EXCELENCIA</h3>
				<img src="img/conocenos/medioaambiente.jpg" alt="Diseño de Interiores">
				<p>En la firma MARIANGEL COGHLAN trabajamos para desarrollar nuestras actividades de forma respetuosa con la conservación del medio ambiente, desde la perspectiva de que el cuidado del entorno natural y el desarrollo sostenible proporcionan un valor añadido a nuestras actividades y servicios, por consiguiente a la calidad de vida de nuestros clientes y de la sociedad en general. Estamos comprometidos con el medio ambiente por lo que modernizamos nuestras instalaciones para reducir el consumo de agua, energía eléctrica y todo tipo de materiales de oficina, haciendo conciencia en nuestro equipo para reducir el impacto ambiental.</p>
			</div>		
			<div class="col-md-4">
				<h3>RESPONSABILIDAD SOCIAL</h3>
				<img src="img/conocenos/responsabilidadsocial.jpg" alt="Diseño de Interiores">
				<p>En la firma MARIANGEL COGHLAN tenemos una política corporativa en responsabilidad social, por lo que consideramos uno de los activos más valioso de la empresa, nuestra reputación. Ser una compañía líder, conduciéndonos con honor, y tener el más alto nivel de integridad y ética corporativa, son parte importante de los objetivos de nuestra firma. Mantener esta reputación es una de nuestras tareas más significativas, ello nos exige llevar a cabo nuestra labor viviendo con intensidad nuestros valores apegados a principios éticos, así mismo estamos al pendiente de las necesidades de los que nos rodean.</p>
			</div>		
		</div>
	</div>
</section>
<?php include('global/cta-ebooks.php');?>
<section id="unete">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h2>Únete al equipo</h2>
			</div>
			<div class="col-md-6">
				<h4>Nuestra Firma trabaja diariamente para ser la mejor, por lo mismo solamente contratamos a los mejores. Tú puedes formar parte de este increíble equipo.</h4>
				<p>Vacantes</p>
				<ul>
					<li><a href="/pdf/lider-de-proyecto-vacante-mariangelcoghlan.pdf" target="_blank">Líder de Proyecto</a></li>
					<li><a href="/pdf/disenador-jr.pdf" target="_blank">Diseñador Jr.</a></li>
					<li><a href="/pdf/practicas-vacante-mariangelcoghlan.pdf" target="_blank">Prácticas profesionales</a></li>
					<li><a href="/pdf/AUXILIAR ADMINISTRATIVO.pdf" target="_blank">Auxiliar Administrativo</a></li>
				</ul>
			</div>
			<div class="col-md-6">
				<p>Si estás interesad@ en postularte para ser parte de nuestra firma puedes mandar un mail a <a href="mailto:reclutamiento@mcoghlan.mx"> reclutamiento@mcoghlan.mx</a> con las siguientes características:</p>
				<ol>
					<li>Título del correo: (Según la vacante a la que aspiras)</li>
					<li>Adjuntar CV con foto actualizado</li>
					<li>Link para consultar portafolio</li>
					<li> Cinco líneas como máximo explicando por que habríamos de considerarte para formar parte de nuestro equipo.</li>
				</ol>
				<p>Antes de enviar un correo asegúrate de cumplir con todos los requisitos.</p>
			</div>
		</div>
	</div>
</section>
<?php include('global/cta-visita.php');?>	
<?php 
include('global/footer.php');
?>	

    